...
RSpec.describe Article, type: :model do
  ...
  describe '.search' do
    ...
    context "valid params" do
      let!(:article_1) { create(:article, category_id: category.id, title: "Виховання дітей", age: "2 місяці", subject: "Сон") }
      ...
      it "search by age" do
        expect(category.articles.search(search: {age: "2 місяці"})[0]).to eq(article_1)
      end

      it "search by subject" do
        expect(category.articles.search("search": {subject: "Сон"})[0]).to eq(article_1)
      end
    end
  end
end
